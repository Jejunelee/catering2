...
import { X, Soup, Salad, Flame, Cake, UtensilsCrossed } from "lucide-react";
...
export default function Premium({ open, onClose }: PremiumProps) {
  if (!open) return null;

  const menuSections = [
    {
      category: "APPETIZER AND SALAD",
      icon: <Salad size={18} />,
      items: [
        "Caviar Pie and Country Chicken Liver Pate with Root Chips and Crostini",
        "Caesar Salad Station",
        "Crisp Greens, Bacon, Parmesan, Herbed Croutons"
      ]
    },
    {
      ...
      icon: <Flame size={18} />,
      items: [
        "Slow Roasted Beef with Bordelaise Sauce",
        "Prawns ala Thermidor",
        "Chicken Pastel",
        "Roasted Vegetables",
        "Mashed Potato, Steamed Jasmine Rice",
        "Pasta Bar - Alfredo, Fresh Tomato, Pesto"
      ]
    },
    {
      category: "DESSERT",
        ...
        <div className="relative w-full h-[200px] md:h-[280px] flex-shrink-0">
          ...
          <div className="absolute bottom-0 w-full flex justify-center">
            <div className="bg-white text-[#F26522] text-sm md:text-xl tracking-wide px-4 md:px-8 py-2 md:py-3 mb-3 md:mb-4 font-semibold rounded-full md:rounded-none shadow-lg md:shadow-none">
              2,200 + 5% SERVICE CHARGE • MINIMUM OF 50 PAX
            </div>
          </div>
